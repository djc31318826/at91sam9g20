...
#define CONFIG_SYS_MACB_TX_TIMEOUT		10000//1000
...
unsigned long sendcount=0,rcvcount=0;
...
static int macb_recv(struct eth_device *netdev)
{
	...
	u32 status;
//printf("0-rcvcount=%d\r\n",rcvcount++);
	//for(length=0;length<10000000;length++);
//for(rcvcount=0;rcvcount<8000;rcvcount++)
//for(length=0;length<0x7fffffff;length++);
	for (;;) {
		if (!(macb->rx_ring[rx_tail].addr & RXADDR_USED))
{
//printf("0-rcvcount=%d\r\n",rcvcount++);
//for(rcvcount=0;rcvcount<5000/*0x800000*/;rcvcount++)
//for(length=0;length<0x7fffffff;length++);
	//rcvcount++;			//1000000,100000000 nok
			return -1;
...
//{
	...
	phy_id = macb_mdio_read(macb, MII_PHYSID1);
//	printf("phy_id=%d,macb->phy_addr=%d\r\n",phy_id,macb->phy_addr);
//}
...
///////////////////////////////////////////////////////////////////////////////
	...
	printf("Force Net to 100M FULL\r\n");
	printf("MII_BMCR=%x\r\n",macb_mdio_read(macb, MII_BMCR));	
	macb_mdio_write(macb, MII_BMCR, 0x2100);	
	printf("MII_BMCR=%x\r\n",macb_mdio_read(macb, MII_BMCR));
	printf("MII_BMSR=%x\r\n",macb_mdio_read(macb, MII_BMSR));
	status=macb_mdio_read(macb, 16);
	printf("mii[16]=%x\r\n",status);
	macb_mdio_write(macb, 16, 0x1594);
	status=macb_mdio_read(macb, 16);
	printf("mii[16]=%x\r\n",status);
