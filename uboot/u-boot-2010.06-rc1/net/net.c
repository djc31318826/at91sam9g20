...
#include "dns.h"
#endif
//#define	DEBUG   //add by djc001 @20110103
...
NetSendUDPPacket(uchar *ether, IPaddr_t dest, int dport, int sport, int len)
{
	...
	if (memcmp(ether, NetEtherNullAddr, 6) == 0) {
		...
		pkt += NetSetEther (pkt, NetArpWaitPacketMAC, PROT_IP);	//{0,0,0,0,0,0}
/*
int i;
for(i=0;i<6;i++)
printf("NetArpWaitPacketMAC[%d]=%d\r\n",i,NetArpWaitPacketMAC[i]);
*/
...
int i=0;
printf("\r\n");
for(i=0;i<(pkt - NetTxPacket) + IP_HDR_SIZE + len;i++)
{
	printf("%2.2x ",NetTxPacket[i]);
}
printf("\r\n");
*/
...
printf("\r\n");
printf("&et->et_src[5]=%x,&et->et_protlen=%x\r\n",&(et->et_src[5]),&(et->et_protlen));
...
{
	volatile uchar *p=(volatile uchar *)0x20000000;
	int i=0,j=0;
	NetInitLoop(0);	
	NetOurEther[0]=0x0;
	NetOurEther[1]=0x23;
	...
	NetTxPacket = &PktBuf[0] + (PKTALIGN - 1);
	NetTxPacket -= (ulong)NetTxPacket % PKTALIGN;
printf("NetTxPacket=%8.8x\r\n",NetTxPacket);
	//uchar ethaddr[6]={0};//{0x00,0x19,0xdb,0xb6,0x2f,0x3c};
	uchar ethaddr[6]={0x00,0x19,0xdb,0xb6,0x2f,0x3c};
	//read data from fpga;
	//start sample
	/*
	#define	smp_sel   		0x28
	...
	*/
	...
	fpga_write(smp_sel,1);	//smp_sel=1;
//printf("(PKTBUFSRX+1) * PKTSIZE_ALIGN + PKTALIGN=%d,PKTBUFSRX=%d\r\n",(PKTBUFSRX+1) * PKTSIZE_ALIGN + PKTALIGN,PKTBUFSRX);


/*
	fpga_write(smp_start,0);
	fpga_write(smp_start,1);	
*/
//for(i=0;i<)
cntl=0;
	for(j=0;j<256;j++)
	{
		...
		read_tmp=fpga_read(rd_smp_datal);	
		NetTxPacket[42+j*4]=read_tmp;
		NetTxPacket[42+j*4+1]=read_tmp>>8;
		
		//if(read_tmp>=2048)
		//	read_tmp=read_tmp-4096;
		//printf("%d\r\n",read_tmp);
		read_tmp=fpga_read(rd_smp_datah);
		NetTxPacket[42+j*4+2]=read_tmp;
		NetTxPacket[42+j*4+3]=read_tmp>>8;
		
		/*
		cntl++;
		NetTxPacket[42+j*4+3]=cntl>>24;
		NetTxPacket[42+j*4+2]=(cntl>>16)&0xff;
		NetTxPacket[42+j*4+1]=(cntl>>8)&0xff;
		NetTxPacket[42+j*4]=(cntl>>0)&0xff;
		*/
	}

	//NetSendUDPPacket(uchar *ether, IPaddr_t dest, int dport, int sport, int len)
	NetSendUDPPacket(ethaddr,50440384,3048,1701,1024);
	//printf("rd_addr =%d\r\n",fpga_read(0x30));
	printf("rd_addr =%x\r\n",fpga_read(0x30));	
/*
NetTxPacket[0 ]=0x00;
...
NetTxPacket[44]=0x31;
*/	
	//NetSendUDPPacket(ethaddr,0x301a8c0,3048,1701,46);//
...
int do_sends ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	volatile uchar *p=(volatile uchar *)0x20000000;
	int i=0,j=0;
	NetInitLoop(0);	
