...
int __init at91_add_device_hdmac(void)
{
...
#else
int __init at91_add_device_hdmac(void) {return 0;}
#endif
...
void __init at91_add_device_mci(short mmc_id, struct mci_platform_data *data)
{
	...
	if (data->dma_slave)
		atslave = kmemdup(to_at_dma_slave(data->dma_slave),
				sizeof(struct at_dma_slave), GFP_KERNEL);
	else
		atslave = kzalloc(sizeof(struct at_dma_slave), GFP_KERNEL);

	data->dma_slave = &atslave->slave;
	/* DMA slave channel configuration */
	atslave->slave.dma_dev = &at_hdmac_device.dev;
	atslave->slave.reg_width = DMA_SLAVE_WIDTH_32BIT;
	...
	if (mmc_id == 0) {		/* MCI0 */
		...
		/* DMA slave channel configuration for MCI0 */
		atslave->slave.dev = &at91sam9g45_mmc0_device.dev;
		atslave->cfg |= ATC_SRC_PER(0) | ATC_DST_PER(0);
		...
		/* DMA slave channel configuration for MCI1 */
		atslave->slave.dev = &at91sam9g45_mmc1_device.dev;
		atslave->cfg |= ATC_SRC_PER(13) | ATC_DST_PER(13);
...
#if defined(CONFIG_MACH_AT91SAM9G45EKES_REV_C) || defined(CONFIG_MACH_AT91SAM9M10G45EK)
...
postcore_initcall(at91_add_device_hdmac);
