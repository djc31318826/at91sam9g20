...
static irqreturn_t atmel_tsadcc_interrupt(int irq, void *dev)
{
	...
	if (status & ATMEL_TSADCC_NOCNT) {
		...
		if (cpu_is_at91sam9g45())
			reg = atmel_tsadcc_read(ATMEL_TSADCC_MR) | (0xD << 28);
		else
			reg = atmel_tsadcc_read(ATMEL_TSADCC_MR) | (0xF << 28);
...
#if 1 /*TODO: saturate if this value is too high */
	 if (prsc > 0xff)
		 prsc = 0xff;
#endif
...
if(cpu_is_at91sam9g45()){
	reg = ATMEL_TSADCC_TSAMOD_TS_ONLY_MODE		|
		...
		((0x00 << 7) & ATMEL_TSADCC_PRES)	|		/* Enable Pressure Measurement */
		((prsc << 8) & 0xFF00)			|		/* PRESCAL, NEEDS TO BE DEFINED FOR THE 9G45(=FF00)*/
		((0x26 << 16) & ATMEL_TSADCC_STARTUP)|			/* STARTUP */
		((0x0d << 28) & ATMEL_TSADCC_PENDBC);			/* PENDBC */
}
else
{
	reg = ATMEL_TSADCC_TSAMOD_TS_ONLY_MODE		|
	...
	atmel_tsadcc_write(ATMEL_TSADCC_MR, reg);
	atmel_tsadcc_write(ATMEL_TSADCC_TRGR, ATMEL_TSADCC_TRGMOD_NONE);
	atmel_tsadcc_write(ATMEL_TSADCC_TSR, (0x0A << 24) & ATMEL_TSADCC_TSSHTIM);
